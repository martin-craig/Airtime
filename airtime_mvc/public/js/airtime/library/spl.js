...
var AIRTIME = (function(AIRTIME){
    ...
    function changeCues($el, id, cueIn, cueOut) {
        ...
        var url = BaseDir+"Playlist/set-cue",
            lastMod = getModified(),
            type = $('#obj_type').val(),
            li,
            span;
    ...
    function changeCrossfade($el, id1, id2, fadeIn, fadeOut, offset, id) {
        ...
        var url = BaseDir+"Playlist/set-crossfade",
            lastMod = getModified(),
            type = $('#obj_type').val();
	...
	function editName() {
    	...
    	url = BaseDir+'Playlist/set-playlist-name';
		...
		$pl.delegate(".spl_block_expand",
		        {"click": function(ev){
		            ...
		            if ($(this).hasClass('close')) {
                        var sUrl = BaseDir+"playlist/get-block-info";
                        mod.disableUI();
	...
	function initialEvents() {	
		...
		$pl.on("click", "#spl_crossfade", function() {
	        ...
	        if ($(this).hasClass("ui-state-active")) {
	            ...
	            var url = BaseDir+'Playlist/get-playlist-fades';
		        $.post(url, 
		...
		$pl.on("blur", "span.spl_main_fade_in", function(event){
		    ...
		    var url = BaseDir+"Playlist/set-playlist-fades",
			    span = $(this),
			    fadeIn = $.trim(span.text()), 
			    lastMod = getModified(),
	            type = $('#obj_type').val();
		...
		$pl.on("click", "#webstream_save", function(){
            ...
            var url = BaseDir+'Webstream/save';
            $.post(url, 
        ...
        $lib.on("click", "#pl_edit", function() {
            openPlaylistPanel();
            $.ajax( {
                url : BaseDir+"usersettings/set-library-screen-settings",
                type : "POST",
                data : {
                    settings : {
                        playlist : true
        ...
        $('#save_button').live("click", function(event){
            ...
            var criteria = $('form').serializeArray(),
                ...
                save_action = BaseDir+'Playlist/save',
                obj_id = $('input[id="obj_id"]').val(),
        ...
        $("#pl-bl-clear-content").live("click", function(event) {
            var sUrl = BaseDir+"playlist/empty-content",
                oData = {};
	...
	mod.fnDeleteItems = function(aItems) {
		var sUrl = BaseDir+"playlist/delete-items",
			oData = {"ids": aItems};
